...
def generate_combat_narrative(result: dict) -> str:
    ...
    narratives = {
        "decisive_victory": [
            ...
            f"{name} catches the first one mid-lunge and redirects its momentum into a concrete wall. The skull pops like a stepped-on grape — grey matter and bone fragments paint the brick in a fan pattern. The second one trips over its own entrails — its abdomen was already split open, guts trailing behind it like a fleshy anchor. {name} stomps the base of its spine and the body folds in half the wrong way. The third runs. {name} lets it. It won't get far with half a leg.",
        ],
        "victory": [
            ...
            f"One of them bites {name}'s hand — teeth clamping down on the meaty part of the palm, grinding through skin and into the muscle beneath. {name} howls and slams the zombie's head against a car door. Once. Twice. On the third hit, the jaw finally loosens — along with three of its teeth, still embedded in {name}'s flesh like yellowed seeds. Blood pours from the wound, mixing with the black tarry fluid leaking from the zombie's crushed skull. {name} picks the teeth out one by one, hissing through clenched jaw, and wraps the hand tight. The bandage is soaked through in seconds.",
        ],
        "pyrrhic": [
            ...
            f"One of them opens {name} up. Not a bite — a swipe, from hip to ribs, deep enough that you can see the fat layer beneath the skin part like curtains, glistening yellow-white before the blood fills in. {name} looks down at the wound and makes a sound that isn't a scream — it's a gasp, a choked denial, one hand pressing against the gash trying to hold everything inside where it belongs. Intestines bulge against {name}'s fingers, slick and hot and trying to escape. {name} fights one-handed after that, the other arm locked across the wound, and somehow — God knows how — puts the last two down. Collapses against the bus. Elena runs out with the medical kit. Nobody speaks. The only sound is {name}'s breathing and the wet sound of things being pushed back inside.",
        ],
        "defeat": [
            ...
            f"It's a feeding. That's the only word for it. They drag {name} down and the first one buries its face in {name}'s stomach — not biting, BURROWING, jaw working like a dog with a bone, teeth tearing through shirt and skin and the muscle wall beneath. {name}'s scream hits a pitch that doesn't sound possible from a human throat. Another one latches onto {name}'s calf and SHAKES — the leg whips back and forth, the knee bends sideways, and something in the joint gives with a pop you can hear over the screaming. By the time you get there with the rifle, {name} is lying in a spreading lake of their own blood, hands pressed against a hole in their abdomen that you can see daylight through. You carry them to the bus. They're lighter than they should be. Some of {name} is still on the pavement.",
        ],
    }
